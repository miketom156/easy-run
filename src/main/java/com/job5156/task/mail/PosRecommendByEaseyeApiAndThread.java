...
public class PosRecommendByEaseyeApiAndThread extends BaseMailSend {
    private static Logger logger = Logger.getLogger(PosRecommendByEaseyeApiAndThread.class);

    private Gson gson;
    private EmailSender sender;
    private int sendLimitNum; //每天最多发送的数量
    private int pageSize = 1000;
    private static boolean TEST_MODE = false;   //测试模式
    private int threadCount;// 定义最大启动线程个数
    public  static Map<String, Integer> statusMap = Maps.newHashMap();


    public PosRecommendByEaseyeApiAndThread() throws EmailServerConfigException {
        gson = new Gson();
        sender = new EmailSender(MailServer.THIRD_EASEYE);
    }

    public PosRecommendByEaseyeApiAndThread(MailServer mailServer) throws EmailServerConfigException {
        gson = new Gson();
    ...
    public static void main(String[] args) {
        try {
            PosRecommendByEaseyeApiAndThread recommend = new PosRecommendByEaseyeApiAndThread();
            recommend.setTestMailAddr("yf_198407@163.com");
            recommend.setSendLimitNum(3);
            recommend.setThreadCount(10);
            TEST_MODE = true;
            recommend.send();
    ...
    public void sendMailByIntervalDays(Date date, Integer intervalDays, Integer taskId, String proc) {
        ...
        if (allRecord > 0) {
            ...
            try {
                for (int i = 0; i < allPage; i++) {
                    ...
                    //构建邮件内容
                    this.sendMail(userList);
                    //调用线程发送
                    callThreadToSend();
                    //中止超时的线程(保险起见再杀一次：）
                    ThreadUitl.killThreadWhenOvertime(3);
                    session.clear();
                }
        ...
        if ((countSendNum + 5000) >= allRecord) { //因为6天之后，应发送总数会有变更化，故已发送数+5000
     ...
     */
    private void sendMail(List<PerUser> userList) throws IOException {

        if (CollectionUtils.isNotEmpty(userList)) {
            int count = 0;
            for (PerUser perUser : userList) {
                if (count >= sendLimitNum) {
                    break;
                }
...
/*                    try {
                    ...
                    vo.setEmail(TEST_MODE ? this.getTestMailAddr()[0] : email);
                    vo.setContent(mailContent);
                    vo.setPerUserId(perUser.getId());
                    ThreadUitl.addObject(vo); //数据放入LIST池中,以备线程共用
     ...
     * 调用线程
     */
    public void callThreadToSend(){

        for(Object emailvo : ThreadUitl.list) {
            ...
            try {
                ...
                while (tCount < threadCount && tCount < sendLimitNum) {
                    // System.out.println("线程个数:" + threadCount);
                    sendEmailByTread sendEmailByTread = new sendEmailByTread();
                    Thread thread = new Thread(sendEmailByTread);
//thread.setDaemon(true);
//这里在tomcat或者其它的web应用程序方面启动线程时应该设置为true.
// 在tomcat 关闭的时候,线程也应该关闭.因为用户线程是基于jvm的,如果不设置为true(守护线程),
// tomcat关闭的时候,线程还会在运行,那会消耗cpu 与内存的.
                ...
                if (this.getSendNum() % this.getSendTestMailPerNum() == 0) {
                    EmailVo vo = (EmailVo)emailvo;
                    sender.send(new PosRecommendMailVo(vo.getContent(), this.getTestMailAddr(), Constants.MAIL_TYPE_SIMPLIFIED));
                }
     ...
     * 通过线程调用API发送邮件
     */
    class sendEmailByTread extends Thread {
        ...
        public void run() {
            while (!isInterrupted) {
               // System.out.printf("Thread is start....");
                try {
                    while (ThreadUitl.list.size() > 0) {
                        EmailVo vo = new EmailVo();
                        synchronized (ThreadUitl.list) {
                            vo = (EmailVo) ThreadUitl.list.get(0);
                            ThreadUitl.removeObject();
                            ThreadUitl.runThreadMap.put(this, new Date().getTime()); //记录线程的开始时间
                        }
                        boolean isSend = sender.send(new PosRecommendMailVo(vo.getContent(), new String[]{vo.getEmail()}, Constants.MAIL_TYPE_SIMPLIFIED));
                        if (isSend) {
                            synchronized (ThreadUitl.list) {
                                addMailCount(statusMap, MailCount.SEND_SUCCESS);
                                if (getSendNum() % 100 == 0) {
                                    updateLogMailBathSend(statusMap.get("id"), vo.getPerUserId(), getSendNum(), Constants.MAIL_SEND_WAIT); //记录当前PERUSER ID
                                }
                            }
                        }else{
                            addMailCount(statusMap, MailCount.SEND_FAIL);
                        }
                        interrupt();
                    }
    ...
    class EmailVo {
        ...
        Integer getPerUserId() {
            return perUserId;
        }

        void setPerUserId(Integer perUserId) {
