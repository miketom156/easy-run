...
import com.job5156.common.validator.DateFormat;
import com.job5156.common.validator.EmailFormat;
import com.job5156.common.validator.MobileFormat;
import com.job5156.model.per.PerUser;
import com.job5156.vo.sys.SysResumeOperatorHistoryVo;
...
//	private String address;               //详细地址
	...
	private AddressVo addressVo;		  //地址vo 
	private AddressVo addressVoEn;		  //英文地址vo
	private List<SysResumeOperatorHistoryVo> resumeHistoryVoList = Lists.newArrayList();//简历审核历史记录vo列表
	
    private transient PerUser mappingModel;
    //private transient Integer perArrival =1;   //到岗时间 (显示用)
    ...
    public PerUserVo(PerUser perUser) {
        if(perUser != null) {
            ...
            if(validator.validate(perUser.getAddress()) && JsonValidator.validJsonToVo(perUser.getAddress(), new TypeToken<AddressVo>(){})) {
   			 	this.setAddressVo(gson.fromJson(perUser.getAddress(), AddressVo.class));
            }else{
            	addressVo = new AddressVo();
            	addressVo.setAddr(perUser.getAddress());
            }
            
            if(validator.validate(perUser.getAddressEn()) && JsonValidator.validJsonToVo(perUser.getAddressEn(), new TypeToken<AddressVo>(){})){
            	this.setAddressVoEn(gson.fromJson(perUser.getAddressEn(), AddressVo.class));
            }else{
            	addressVoEn = new AddressVo();
            	addressVoEn.setAddr(perUser.getAddressEn());
            }
...
/*    public Integer getPerArrival() { //取得到岗时间（类别）
        if (perArrival == null || perArrival < 0) {
            PerResumeVo resumeVo = getDefaultPerResumeVo();
            if (resumeVo != null) {
                PerResumeVo.IntentInfoVo intentInfoVo = resumeVo.getIntentInfoVo();
                if (intentInfoVo != null) {
                    setPerArrival(NumberUtils.toInt(intentInfoVo.getCheckindate()+"", 0));
                }
            }
        }
        return perArrival;
    ...
    public String getDegreeStr() {
        if(degree != null ) {//有值直接转换
            return OptionMap.getValueByType(OptionType.OPT_PER_DEGREE, degree, null);
        }else{//从默认简历的学历列表转换
            PerResumeVo resumeVo = getDefaultPerResumeVo();
            if (resumeVo != null) {
                List<PerEducationInfoVo> eduList = resumeVo.getEducationInfoVoList();
                if(eduList.size() > 0){//对学历进行排序
                    Collections.sort(eduList,new Comparator<PerEducationInfoVo>() {
                        @Override
                        public int compare(PerEducationInfoVo o1, PerEducationInfoVo o2) {
                            return o1.getDegree().compareTo(o2.getDegree());
                        }
                    });
                }
                return eduList.get(0).getDegreeStr();
            }
        }
        return "";
    }
    ...
    public String getAddressStr(){
        return addressVo != null ? addressVo.toString() : "";
    }
	...
	public List<SysResumeOperatorHistoryVo> getResumeHistoryVoList() {
		return resumeHistoryVoList;
	}

	public void setResumeHistoryVoList(
			List<SysResumeOperatorHistoryVo> resumeHistoryVoList) {
	...
	public AddressVo getAddressVoEn() {
		return addressVoEn;
	}

	public void setAddressVoEn(AddressVo addressVoEn) {
		this.addressVoEn = addressVoEn;
	}
